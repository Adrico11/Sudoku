# -*- coding: utf-8 -*-
"""
Created on Wed Apr 22 11:17:15 2020
...
def create_image_gridGif(grid,nb,pos,colour,t):
    ...
    for i in range(L):
        for j in range(L):
            if grid[i][j]!=0:
                draw.text(((j+0.3)*step_size, (i+0.2)*step_size),str(grid[i][j]),(0,0,0),font=font)
    draw.text(((pos[1]+0.25)*step_size,(pos[0]+0.25)*step_size), str(nb),colour,font=font)        
    image.save(r'C:\Users\snice\OneDrive\Bureau\Computer Stuff\Python Projects\Sudoku\SudokuSteps\Gif'+str(t)+'.png',"PNG")                
    ...
    if not find: #if find==None
        ...
        for nb in range(1,10):
            if is_valid(grid,nb,(a,b)):
                ...
                pos=(a,b)
                t=time.time()
                create_image_gridGif(grid,nb,pos,(255,0,0),t)
                draw_grid(grid)
                if solveGif(grid):
                    return True
                t1=time.time()
                create_image_gridGif(grid,'X',pos,(255,0,0),t1)
                grid[a][b]=0
...
def makeGif():
    """Renvoi un Gif du parcours de l'algo"""
    filenames = sorted(fn for fn in os.listdir(r'C:\Users\snice\OneDrive\Bureau\Computer Stuff\Python Projects\Sudoku\SudokuSteps') if fn.endswith('.png'))
    images = []
    print(filenames)
    for filename in filenames:
        images.append(imageio.imread(filename))
    imageio.mimsave(r'C:\Users\snice\OneDrive\Bureau\Computer Stuff\Python Projects\Sudoku\Gif.gif', images, duration=0.1)
    #shutil.rmtre(r'C:Users\snice\OneDrive\Bureau\Computer Stuff\Python Projects\Lorentz\Images')
     
    
    
def resolutionGif(grid):
    """Fonction d'affichage complet"""
    t=time.time()
    create_image_gridGif(grid,1,(0,0),(255,255,255),t)
    print(solveGif(grid))
    makeGif()
